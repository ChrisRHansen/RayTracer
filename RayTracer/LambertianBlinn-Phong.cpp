...
void LambertianBlinn_Phong::Run()
{
	int numPixX = 800;
	int numPixY = 600;
	int left = -(numPixX / 2);
	...
	//////////////////////////////////////
	Color specularColor(255, 255, 255); //
		...
		for (int j = 0; j < numPixY; j++)
		{
			for (SceneSurface* object : objects)
			{
				...
				if (testResult.m_bRayHit)
				{
					...
					Vector3f half_vector = ( (view_direction + light_direction) / ( (view_direction + light_direction).magnitude() ) ).normalized();

					image[j][i] = object->color * intensity * fmax(0, surface_normal * light_direction) // Lambertian
						+ specularColor * intensity * pow( fmax(0, surface_normal * half_vector), phongExponent ); // Blinn-Phong
				}
			}
		}
	}
